# <div style="text-align: right">Machine Learning - February 27, 2020</div>
...
### Homework:
- Assginment 1 Due Tuesday
---
## I. Pandas
- This is a short introduction to pandas, geared mainly for new users.
- You can see more Complex recipes in the Cookbook, https://pandas.pydata.org/pandas-docs/stable/user_guide/cookbook.html#cookbook 
...
dtype: object
```
- If you're using IPthyon, tab completion for column names (as well as public attributes) is automatically enabled. Here's a sub of the attributes that will be completed:
```python
...
df2.D
```
- As you can see, the columns `A`, `B`, `C`, and `D` are automatically tab completed.
...
- Note that this can be an exprensive operations when your `DataFrame` has columns with different data types, which comes down to a fundamental difference between pandas and NumPy.
    - NumPy arrays have one dtype forthe entire array, while pandas DataFrames have one dtype per column.
...
```
- NOTE:
    - `DataFrame.to_numpy()` does *not* include the index or clumn labels in the output.
...
## II. Selection
- NOTE:
    - While standard Python / Numpy expressions for selecting and setting an intuitive and come in handy for interactive work, for production code, we recommend the optimized pandas data access methods, `.at`, `.iat`, `.loc`, and `iloc`.
    - See the indexing documentation `Indexing and Selecting Data` and `MultIndex / Advanced Indexing`. 
- Getting:
...
2013-01-05 -0.424972  0.567020
```
- By lists of integers position locations, similar to the numpy / python style:
```python 
...
```
- Boolean Indexing
    - Using a single clumn's values to select data.
```python
In [39]: df[df['A'] > 0]
Out[39]: 
                   A         B         C         D
...
2013-01-04  0.721555 -0.706771 -1.039575  0.271860
```
- Selecting Values from DataFrame where a boolean condition is met
```python 
In [40]: df[df > 0]
Out[40]: 
                   A         B         C         D
...
- It is by default not included in computations.
    - See the missing data section, https://pandas.pydata.org/pandas-docs/stable/user_guide/missing_data.html#missing-data
...
```
- to get the boolean mask where vales are `nan`:
```python
In [60]: pd.isna(df1)
Out[60]: 
                A      B      C      D      F      E
...
- In addition, pandas automatically broadcasts along the specified dimensions.
```python
...
In [135]: ts.plot()
Out[135]: <matplotlib.axes._subplots.AxesSubplot at 0x7fc69a6e5390>
...
In [139]: df.plot()
Out[139]: <matplotlib.axes._subplots.AxesSubplot at 0x7fc69a3b7a50>

In [140]: plt.legend(loc='best')
Out[140]: <matplotlib.legend.Legend at 0x7fc69a6300d0>
```
